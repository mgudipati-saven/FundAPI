  ...
  puts "Processing the fund basic data file: #{$infile}..."
	CSV.foreach($infile, :quote_char => '|', :col_sep =>'|', :row_sep =>:auto) do |row|
	  ...
	  if row[1] == "2"
	    ...
	    fticker = row[4]
	    if fticker
        # add to a sorted set of fund tickers...
        dbkey = "fund.tickers"
	    ...
	    if fticker
	      date = row[3]
	      if date
          dbkey = "fund:#{fticker}:TotalNetAssets"
          $redisdb.hset dbkey, date, row[4]
          
          dbkey = "fund:#{fticker}:Turnover"
          $redisdb.hset dbkey, date, row[5]

          dbkey = "fund:#{fticker}:TotalReturns"
          $redisdb.hset dbkey, date, row[6]
        end
	    end
	  end
	end # CSV.foreach
